...
const OSCOMP_SUITE_SCRIPT: &str = concat!(
    ...
    "    case_timeout_s=\"${WHUSE_LTP_CASE_TIMEOUT:-45}\"\n",
    "    case \"$case_timeout_s\" in\n",
    "        ''|*[!0-9]*) case_timeout_s=45 ;;\n",
    "    esac\n",
    "    if [ \"$case_timeout_s\" -le 0 ]; then\n",
    "        case_timeout_s=1\n",
    "    fi\n",
    "    if [ -z \"${whuse_ltp_epoch_counter:-}\" ]; then\n",
    "        whuse_ltp_epoch_counter=0\n",
    "    fi\n",
    "    whuse_ltp_epoch_counter=$((whuse_ltp_epoch_counter + case_timeout_s))\n",
    ...
    "    export WHUSE_LTP_TMPDIR=\"${WHUSE_LTP_TMPDIR:-/tmp}\"\n",
    "    export WHUSE_LTP_RUNROOT=\"${WHUSE_LTP_RUNROOT:-$WHUSE_LTP_TMPDIR}\"\n",
    "    export WHUSE_LTP_WRAPPER_DIR=\"${WHUSE_LTP_WRAPPER_DIR:-}\"\n",
