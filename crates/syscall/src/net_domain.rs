...
pub(crate) fn dispatch(
    ...
    Some(match sysno {
        ...
        SYS_ACCEPT | SYS_ACCEPT4 => ctx.dispatcher.sys_accept(args, ctx.procs, ctx.vfs),
        SYS_CONNECT => ctx.dispatcher.sys_connect(args, ctx.procs, ctx.vfs),
        SYS_GETSOCKNAME | SYS_GETPEERNAME => ctx.dispatcher.sys_getsockname(args, ctx.procs),
        SYS_SENDTO => ctx.dispatcher.sys_sendto(args, ctx.procs, ctx.vfs),
        SYS_RECVFROM => ctx.dispatcher.sys_recvfrom(args, ctx.procs, ctx.vfs),
        SYS_SETSOCKOPT => ctx.dispatcher.sys_setsockopt(args),
        SYS_GETSOCKOPT => ctx.dispatcher.sys_getsockopt(args, ctx.procs),
        SYS_SHUTDOWN => ctx.dispatcher.sys_shutdown(args),
        SYS_SENDMSG => ctx.dispatcher.sys_sendmsg(args, ctx.procs, ctx.vfs),
        SYS_RECVMSG => ctx.dispatcher.sys_recvmsg(args, ctx.procs, ctx.vfs),
        _ => return None,
    })
}
