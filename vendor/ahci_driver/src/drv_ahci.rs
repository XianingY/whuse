...
fn ahci_host_init(ahci_dev: &mut ahci_device) -> i32 {
    ...
    for i in 0..ahci_dev.n_ports {
        ...
        if timeout <= 0 {
            ahci_printf(b"spin up cannot finish\n\0" as *const u8);
            return -1;
        }
        ...
        if timeout <= 0 {
            ahci_printf(b"port %u sata link timeout\n\0" as *const u8, i as u32);
        } else {
            ahci_printf(b"port %u sata link up\n\0" as *const u8, i as u32);
        }
...
fn ahci_exec_ata_cmd(
    ...
    if cmd_slot == 32 {
        ahci_printf(b"cannot find empty command slot\n\0" as *const u8);
        return 0;
    }
...
fn ahci_port_start(ahci_dev: &mut ahci_device, port: u8) -> i32 {
    ...
    if (port_status & 0xf) != 0x3 {
        ahci_printf(b"no link on port %u\n\0" as *const u8, port as u32);
        return -1;
    }
...
fn ahci_port_scan(ahci_dev: &mut ahci_device) -> i32 {
    ...
    if linkmap == 0 {
        ahci_printf(b"no port device detected\n\0" as *const u8);
        return -1;
    }
    ...
    for i in 0..ahci_dev.n_ports {
        if (linkmap >> i & 0x1) != 0 {
            if ahci_port_start(ahci_dev, i) != 0 {
                ahci_printf(b"cannot start port %u\n\0" as *const u8, i as u32);
                return -1;
            }
            ahci_dev.port_idx = i;
            break;
